...
The Gitlab CI compies the SCSS files into a single CSS file after each commit. The latest build is https://gitlab.univ-nantes.fr/bousse-e/stereopticon/-/jobs/artifacts/master/download?job=keep[available here].
...
The theme is written in SCSS, and can be compiled into CSS using the confirmation that already exists for compiling the official reveal.js themes.

Instructions:

- Clone this repository.
- Download reveal.js or use an existing reveal.js installation folder.
- Copy the `css` and `lib` folders of the stereopticon repository inside the reveal.js installation folder, and say *yes* when asking to merge the folders.
- Go in the reveal.js folder and run: `npm install && npm run build -- css-themes`, which will produce `revealjs/css/theme/stereopticon.css`.
...
- Open `http://localhost:8080/demo.html` which should show the stereopticon demo.
